...
var EVENT_DE_STATUS_PREVIOUS = "k95lcIlS4bv";
var EVENT_DE_DAYS_IN_PREV = "UrD7yr6JLEf";

// ==== DataSet =====
var DATASET_UID_STICKY_STATUS_LOG = "mdYzjEg88qe"; // orion server DS id

// ==== OrgUnit Groups UID ====
var OUG_UID_FRANCHISEE_ON_BOARDING = "bieIPjDl9rK";
var OUG_UID_FRANCHISEE_UNDER_CONTACT = "InJBtUUZy2c";
var OUG_UID_FRANCHISEE_SUSPENDED = "f2sBZWvPAAM";
var OUG_UID_FRANCHISEE_DEFRANCHISED = "jyhbiSfzbU5";
var OUG_UID_NETWORK_INCLUDED = "qZzR4nEdbWW";
var OUG_UID_NETWORK_EXCLUDED = "olNLe6DMrgA";
		...
		LogProgram.createLog( _sourceType, function( logCreated, eventLogJson ) 
		{
			...
			{			
				if ( json_SqlViewData !== undefined )
				{
					...
					app.process_OUGroupUpdate( eventList_GroupByOrgUnit );
...
app.updatePrevRelatedData = function( eventList )
{
	for( var i = 0; i < eventList.length; i++ )
	{
		...
		if ( i > 0 )
		{
			...
			var updateDeList = [];
			updateDeList.push( { "deId": EVENT_DE_STATUS_PREVIOUS, "value": eventData.prevSts } );
...
app.process_AggUpdate = function( eventList_GroupByOrgUnit )
{
	...
	for ( ouId in eventList_GroupByOrgUnit )
	{
		try
		{
			...
			app.deleteDataValueSet( DATASET_UID_STICKY_STATUS_LOG, ouId, '1900-01-01', '2500-01-01', function( success )
			{
				var ouEventList = eventList_GroupByOrgUnit[ ouId ];
				var ouEventList_sorted = Util.sortByKey( ouEventList, "eventDate" );

				var event_FirstContract = app.getFirstEvent_UnderContract( ouEventList_sorted );
				...
				for( var i = 0; i < ouEventList_sorted.length; i++ )
				{
					try
					{
						...
						app.submitDataToAggr( eventData, event_FirstContract );
					}
					catch ( ex )
					{
...
app.submitDataToAggr = function( dataJson, event_FirstContract )
{
	var jsonData = [];
	...
	var firstContractEventDate = ( event_FirstContract ) ? event_FirstContract.eventDate.substring(0, 10) : "";
	...
	for ( i = 0; i < 24; i++ )
	{
		...
		jsonData.push( app.getJsonData( AGG_DE_FRANCHISEE_UNKNOWN, ouId, pe, "0" ) );
		jsonData.push( app.getJsonData( AGG_DE_FRANCHISEE_ON_BOARDING, ouId, pe, app.get10_Bool(sts === "ONB") ) );
		
		jsonData.push( app.getJsonData( AGG_DE_FRANCHISEE_UNDER_CONTACT, ouId, pe, app.get10_Bool(sts === "UNC") ) );
		jsonData.push( app.getJsonData( AGG_DE_FRANCHISEE_SUSPENDED, ouId, pe, app.get10_Bool(sts === "SUS") ) );
		jsonData.push( app.getJsonData( AGG_DE_FRANCHISEE_DEFRANCHISED, ouId, pe, app.get10_Bool(sts === "DEF") ) );
		jsonData.push( app.getJsonData( AGG_DE_NETWORK_INCLUDED, ouId, pe, app.get10_Bool(sts === "UNC") ) );
		jsonData.push( app.getJsonData( AGG_DE_NETWORK_EXCLUDED, ouId, pe, app.get10_Bool(sts !== "UNC") ) );
	}
	...
	{
		var pe = app.generateNextPeriodCode( period, i );
		var monthsSinceJoin = ( firstContractEventDate ) ? app.generateMonthSinceDate( firstContractEventDate, pe ) : "";

		jsonData.push( app.getJsonData( AGG_DE_MONTHS_SINCE_JOIN_NETWORK, ouId, pe, monthsSinceJoin ) );
